...
class FormRelationsTiersContacts
{
	 ...
	 */
	public function formconfirm($page, $title, $question, $action, $formquestion = [], $selectedchoice = '', $useajax = 0, $height = 200, $width = 500, $disableformtag = 0)
	{
		global $langs, $conf;
									...
									if ($input['type'] == 'checkbox') {
										...
										if ($input['type'] == 'radio') {
											...
											foreach ($input['values'] as $selkey => $selval) {
												$more .= '<tr>';
												if ($i == 0) {
													$more .= '<td class="tdtop">' . $input['label'] . '</td>';
												} else {
													$more .= '<td>&nbsp;</td>';
												}
											...
											if ($input['type'] == 'date') {
												...
												if ($input['type'] == 'other') {
													$more .= '<tr><td>';
													if (!empty($input['label'])) {
														$more .= $input['label'] . '</td><td colspan="2" align="left">';
													}
													$more .= $input['value'];
													$more .= '</td></tr>' . "\n";
												} else {
													if ($input['type'] == 'onecolumn') {
	...
	public static function show_contacts($conf, $langs, $db, $object, $backtopage = '')
	{
		...
		foreach ($contactstatic->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.' . $key]['checked'])) {
				...
				elseif (in_array($key, array('statut'))) print $form->selectarray('search_status', array(
					'-1' => '',
	...
	public static function show_all_child_contacts($conf, $langs, $db, $object)
	{
		...
		foreach ($contactstatic->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.' . $key]['checked'])) {
				print getTitleFieldOfList($arrayfields['t.' . $key]['label'], 0, $_SERVER['PHP_SELF'], isset($val['sort']) ? $val['sort'] : 't.' . $key, '', $param, ($align ? 'class="' . $align . '"' : ''), $sortfield, $sortorder, $align . ' ') . "\n";
			}
		}
		// Extra fields
		$extrafieldsobjectkey = $contactstatic->table_element;
