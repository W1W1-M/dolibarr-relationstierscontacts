...
class ActionsRelationsTiersContacts
{
	 ...
	 * @return  int				Return <0 on error, 0 on success, 1 to replace standard code
	 */
	 ...
	 * @return  int				Return <0 on error, 0 on success, 1 to replace standard code
	 */
	public function afterSelectOptions($parameters, &$object, &$action, $hookmanager)
	{
		...
		if (in_array('actioncard', $contexts) && in_array('selectcontacts', $contexts)) {
			if ($action == 'create' || $action == 'edit'
				...
				foreach ($contact_ids as $contact_id) {
					...
					if ($showfunction && $contactstatic->poste) {
						$out .= ' (' . $contactstatic->poste . ')';
					}
					if (($showsoc > 0) && $contactstatic->company) {
						$out .= ' - (' . $contactstatic->company . ')';
					}
					$out .= '</option>';
				}
	 ...
	 * @see	restrictedArea()
	 */
	public function checkUserAccessToObject($user, $featuresarray, $objectid = 0, $tableandshare = '', $feature2 = '', $dbt_keyfield = '', $dbt_select = 'rowid')
	{
		global $db, $conf;
		...
		foreach ($featuresarray as $feature) {
			...
			if (in_array($feature, $check)) {
				...
				} elseif (!empty($conf->societe->enabled) && ($user->rights->societe->lire && !$user->rights->societe->client->voir)) { // If internal user: Check permission for internal users that are restricted on their objects
				...
				} elseif (!empty($conf->multicompany->enabled)) { // If multicompany and internal users with all permissions, check user is in correct entity
