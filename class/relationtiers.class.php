...
class RelationTiers extends CommonObject
{
    ...
    /**
     * Create relation thirdparty into database
     *
    ...
    /**
     * Load relation thirdparty in memory from the database
     *
    ...
    /**
     *  Update relation thirdparty into database
     *
    ...
    /**
     * Delete relation thirdparty in database
     *
     ...
     * @param   bool    $notrigger      false=launch triggers after, true=disable triggers
     *
     * @return  int     <0 if KO, 0 or $idActionComm if OK
     */
    private function _createActionComm(User $user, $notrigger = false)
    {
        ...
        if (!empty($actionCommDateP) && (empty($actionCommDateF) || $actionCommDateF>=$actionCommDateP)) {
            ...
            if (is_array($this->sendtoid)) {
                if (count($this->sendtoid) == 1) $contactforaction->fetch(reset($this->sendtoid));
            } else {
                if ($this->sendtoid > 0) $contactforaction->fetch($this->sendtoid);
            }
            // Set societeforaction.
            if ($this->socid > 0) $societeforaction->fetch($this->socid);
     ...
     * @return  int     <0 if KO, array of child id if OK
     *
     * @throws Exception
     */
    ...
    /**
     * Determine if the thirdparty in relation is main thirdparty of the contact
     *
