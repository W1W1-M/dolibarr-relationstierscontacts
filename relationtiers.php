...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if (!empty($id)) {
		...
		if (($action == 'relation_create' || $action == 'relation_edit') && $user->rights->relationstierscontacts->relationtiers->creer) {
			...
			$formConfirmQuestion[] = array(
				'label' => $langs->trans('RTCRelationTiersThirdparty'),
				'type' => 'other',
				'name' => 'relationtiers_socid',
				'value' => $formConfirmSelectThirdparty
			);
		...
		*/
		...
		foreach ($companystatic->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.' . $key]['checked'])) {
				...
				if (in_array($key, array('lastname', 'name'))) {
					print '<input type="text" class="flat maxwidth75" name="search_' . $key . '" value="' . dol_escape_htmltag($search[$key]) . '">';
				} elseif (in_array($key, array('statut'))) {
					print $form->selectarray('search_status', array(
						'-1' => '',
						'0' => $companystatic->LibStatut(0, 1),
						'1' => $companystatic->LibStatut(1, 1)
					), $search_status);
				}
				print '</td>';
			}
		...
		if ($num || (GETPOST('button_search') || GETPOST('button_search.x') || GETPOST('button_search_x'))) {
			...
			while ($i < $num) {
				...
				if (!empty($arrayfields['t.rt_date_debut']['checked'])) {
					...
					if ($relationTiersStatic->date_debut) {
				...
				if (!empty($arrayfields['t.rt_date_fin']['checked'])) {
					print '<td align="center">';
					if ($relationTiersStatic->date_fin) {
				...
				if (!empty($arrayfields['t.rt_commentaire']['checked'])) {
					print '<td>';
					if ($relationTiersStatic->commentaire) {
						print $relationTiersStatic->commentaire;
					}
					print '</td>';
				}
