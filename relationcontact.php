...
if (empty($reshook))
{
    ...
    } else if ($action == 'confirm_edit' &&  $user->rights->relationstierscontacts->relationcontact->creer) {
    ...
    } else if ($action == 'confirm_delete' &&  $user->rights->relationstierscontacts->relationcontact->supprimer) {
...
else
{
    ...
    if (! empty($id))
    {
        ...
        */
        ...
        if ($search_status!='' && $search_status != '-1') $sql1 .= " AND t.statut = ".$db->escape($search_status);
        if ($search_name) $sql1 .= natural_search(array('t.lastname', 't.firstname'), $search_name);
        ...
        if ($search_status!='' && $search_status != '-1') $sql2 .= " AND t.statut = ".$db->escape($search_status);
        if ($search_name) $sql2 .= natural_search(array('t.lastname', 't.firstname'), $search_name);
        // Add where from extra fields
        $extrafieldsobjectkey=$contactstatic->table_element;
        include DOL_DOCUMENT_ROOT.'/core/tpl/extrafields_list_search_sql.tpl.php';
